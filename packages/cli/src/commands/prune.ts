...
export function registerPruneCommand(program: Command) {
  program
    .command('prune')
    .description('Remove all branchdiff data (database, sessions) for all repos')
    .action(() => {
      const dir = join(homedir(), '.branchdiff');
      if (!existsSync(dir)) {
      ...
      console.log(pc.green('Pruned all branchdiff data (~/.branchdiff).'));
    });
}
