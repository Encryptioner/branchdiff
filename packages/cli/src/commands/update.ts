...
export function registerUpdateCommand(program: Command, version: string, skillsHash: string) {
  program
    .command('update')
    .description('Update branchdiff to the latest version')
    .action(() => {
      try {
        const registry = execSync('npm view branchdiff version', { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] }).trim();
        if (registry === version) {
        ...
        execSync('npm install -g branchdiff@latest', { stdio: 'inherit' });
        console.log(pc.green(`Updated to ${registry}.`));

        try {
          const newHash = execSync('branchdiff --skills-hash', { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] }).trim();
        ...
        console.error(pc.red('Failed to update. Try running: npm install -g branchdiff@latest'));
        process.exit(1);
      }
    });
}
