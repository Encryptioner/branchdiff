...
import { isGitRepo, getRepoRoot } from '@branchdiff/git';
import { findInstanceForRepo } from '../registry.js';

export function registerOpenCommand(program: Command) {
  program
    .command('open')
    .description('Open the browser to a running branchdiff instance')
    ...
    .action(async (ref?: string) => {
      ...
      if (!existing) {
        console.error(pc.red('No running branchdiff instance for this repo.'));
        console.log(`Run ${pc.cyan('branchdiff')} to start one.`);
        process.exit(1);
      }
