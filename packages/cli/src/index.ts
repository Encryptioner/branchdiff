...
Common usage:
  ...
  $ branchdiff main                         What changed since main
...
Other commands:
  ...
  $ branchdiff tree --dark                  Browse in dark mode
  $ branchdiff list                         List running instances
  $ branchdiff kill                         Stop all running instances
  $ branchdiff prune                        Remove all branchdiff data
  ...
  .action(async (refs: string[], opts) => {
    ...
    if (opts.base || opts.compare) {
      if (refs.length > 0) {
        ...
        console.log(`  Use either ${pc.cyan('branchdiff --base main --compare feature')} or ${pc.cyan('branchdiff main feature')}, not both.`);
        process.exit(1);
      }
      if (opts.compare && !opts.base) {
        console.error(pc.red('Error: --compare requires --base.'));
        console.log(`  Example: ${pc.cyan('branchdiff --base main --compare feature')}`);
        process.exit(1);
      }
      refs.push(opts.base);
      if (opts.compare) {
    ...
    for (const ref of refs) {
      ...
      if (!isValidGitRef(ref)) {
        ...
        console.log(`  ${pc.cyan('branchdiff')}                              See all uncommitted changes`);
        console.log(`  ${pc.cyan('branchdiff main')}                         What changed since main`);
        console.log(`  ${pc.cyan('branchdiff HEAD~1')}                       Review your last commit`);
        console.log(`  ${pc.cyan('branchdiff main..feature')}                Compare two branches`);
        console.log(`  ${pc.cyan('branchdiff --base main --compare feature')}   Compare two branches`);
        console.log(`  ${pc.cyan('branchdiff staged')}                       Only staged changes`);
        console.log(`  ${pc.cyan('branchdiff tree')}                         Browse repository files`);
        console.log('');
        console.log(`Run ${pc.cyan('branchdiff --help')} for more options.`);
        process.exit(1);
      }
    }
    ...
    if (existing) {
      ...
      if (!isHealthy) {
        ...
        if (!opts.quiet) {
          console.log('');
          console.log(pc.bold('  branchdiff'));
