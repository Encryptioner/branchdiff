...
import { isGitRepo, getDiffFiles, resolveRef } from '@branchdiff/git';
...
function requireSession() {
  ...
  if (!session) {
    ...
    console.error(pc.dim('Start branchdiff first to create a session.'));
    process.exit(1);
  }
  return session;
}
...
Examples:
  ...
  $ branchdiff agent comment --file src/app.ts --line 42 --body "Missing null check"
  $ branchdiff agent resolve abc123 --summary "Added null check"
  $ branchdiff agent reply abc123 --body "Good catch, fixed"
  $ branchdiff agent general-comment --body "Overall this looks good, just a few nits"
  $ branchdiff agent tour-start --topic "How does auth work?" --body "Overview of the auth flow"
  $ branchdiff agent tour-step --tour <id> --file src/auth.ts --line 10 --body "Entry point"
