...
- `mode=file`: blob hash comparison via `getBlobMap()` → `compareBranches()` — skips files with identical content regardless of commit ancestry
- `mode=git` (default): `git diff b1..b2` (two-dot, tip-vs-tip) — surfaces commit-path noise
